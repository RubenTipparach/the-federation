...
### 2.1 A hull is one piece, and that is measured

**Every hull must pass an attachment check and a silhouette check before it is
committed or shown.** A detached pod is the easiest thing to miss from one camera angle
and the easiest to see from another, and the review page found nine of them in one pass
once it started counting. Eyeballing does not find them. Counting does.

- **Attachment.** Take every top level piece of the hull (a part with whatever is mounted
...
- **Silhouette.** Render the hull from directly above at the identification size, 60
  ...
  is the same failure seen from the game's camera, and the fix is a thicker pylon rather
  than a note.
- **Where it runs.** The diorama page reports both counts on every hull and marks a failing
  roster cell. `tools/shiplib.py` runs both on every `.obj` it writes and refuses the
  file, and `tools/check_hulls.py` runs them over the committed meshes so a hand edit in
  Blender is caught too.
